	...
	attackby(W as obj, mob/user as mob)
		if (istype(W, /obj/item/weapon/pen))
			...
			if (t)
				src.name = "key - "
				src.name += t
				src.icon_state = "door_key_label"
			else
				src.name = "key"
			return
		else if(istype(W, /obj/item/weapon/wirecutters))
			user << "You cut the tag off the key"
			src.name = "key"
			icon_state = "door_key"
			return
...
/obj/structure/mineral_door
	...
	proc/TryToSwitchState(atom/user)
		if(isSwitchingStates || locked) return
		if(ismob(user))
			var/mob/M = user
	...
	attackby(obj/item/weapon/W as obj, mob/user as mob)
		if((istype(W, /obj/item/door_key))&&(src.hasLock)&&(!src.state)&&(!src.isSwitchingStates)) //Keys, yeah! -Sbotkin
			if (src.key == W)
				src.locked = !src.locked
				playsound(user.loc, 'sound/machines/door_open.ogg', 100)
				user << (src.locked ? "You locked [src.name]." : "You unlocked [src.name].")
				return
			else
