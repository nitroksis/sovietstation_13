...
/obj/machinery/door/airlock
	...
	var/manualHackProof = 0 //same for manual hacking with tools etc.
...
/obj/machinery/door/airlock/bumpopen(mob/living/user as mob) //Airlocks now zap you when you 'bump' them open when they're electrified. --NeoFite
	if ((issilicon(usr))&&(src.aiControlDisabled))
		return
	if(!issilicon(usr))
...
/obj/machinery/door/airlock/attackby(C as obj, mob/user as mob)
	...
	if((istype(C, /obj/item/weapon/weldingtool) && !( src.operating > 0 ) && src.density))
		...
		if (!src.manualHackProof)
			src.p_open = !( src.p_open )
			src.update_icon()
		else
			user << "Airlock is additionally secured. You cannot unscrew the panel."
		...
		if (!src.hackProof)
			...
			user << "Airlock manual AI control port is secured. Unable to insert."
