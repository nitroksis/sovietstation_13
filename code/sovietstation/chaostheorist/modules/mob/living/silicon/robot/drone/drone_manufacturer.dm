...
#define DRONE_WHITELIST_FILE "data/drone_whitelist.txt"
var/static/list/drone_whitelist = null

/hook/startup/proc/loadDroneWhitelist()
	load_drone_whitelist()
	return 1

/proc/load_drone_whitelist()
	drone_whitelist = file2list(DRONE_WHITELIST_FILE)
	if(!drone_whitelist.len) drone_whitelist = null
...
/mob/dead/verb/join_as_drone()
	...
	if(drone_whitelist)
		if(!(src.ckey in drone_whitelist))
			usr << "\red You are not in the drone whitelist."
			return
	else
		usr << "\red Error. Whitelist not loaded. Report to administration to fix it."
		return
